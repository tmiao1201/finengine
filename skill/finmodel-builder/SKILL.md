...
description: |
  ...
  四层架构：行业配置 → Meta 宽表数据层 → 行业无关逻辑引擎 → 呈现层；配双算交叉验证。
...
```
行业配置层  industry_config.py   ← 实例化参数（AI 编的假设 or 公司真实口径）
     ↓ 初始化
...
**关键承诺：引擎只认 Meta 接口。** 初始化用配置灌 Meta；进公司后用 ERP 导出灌 Meta（scenario=ACTUAL 按期间覆盖）——同一份引擎两段人生。roundtrip 测试证明这件事：从 xlsx 读回 Meta → bind → 重算 → 与基线逐格一致。
...
### Step 1 行业适配（决定 metric 集合）
先判断目标公司的收入是三种模式的哪种组合（可混合）：
...
产出：`metric_dict.py`（每个指标：中文含义、entity 归属、维度适用性、模式归类）+ `industry_config.py`。
